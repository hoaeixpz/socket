...
class MonthlyStrategy(bt.Strategy):
    ...
    def update_code_pool(self, current_year):
        self.code_pool = set()
        current_code_list = code_table.get(current_year)
        if current_code_list is None:
            print(f"Error {current_year} code table is None")
            return

        for data in self.datas:
            code_name = data._name
            if code_name in current_code_list:
                self.code_pool.add(data)
    ...
    def next(self):
        ...
        if year_changed:
            self.update_code_pool(current_year)
    ...
    def stop(self):
        ...
        if len(self.code_pool) < 5:
            initial_cash = initial_cash / 5 * len(self.code_pool)
...
def run_backtest():
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        code_list = code_table.get(year)
        for code in code_list:
            code_set.add(code)

    print(f"Total code {len(code_set)}")

    for code in code_set:
        data_name = load_hfq_data(code)
        from_date = datetime(START_YEAR - 1, 12, 15)
    ...
    qs.reports.html(
        returns,
        output=f'{START_YEAR}-{END_YEAR}_rising_profit_golden.html',
        title='策略分析',
        rf=0.02
    )
