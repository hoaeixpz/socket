...
def calc_R2_weights(ContextInfo):
	for s in stocks:
		down_history_data(s, '1d', "", "")
	...
	for code in stocks:
		...
		if df is None or len(df) < base_days:
			...
			print(f"{code} {ContextInfo.get_stock_name(code)}")
			print(df['close'][-6:])
...
def calc_trades(ContextInfo):
	...
	for code in stocks:
		...
		if price is None:
			print(f"  !! {code} {ContextInfo.get_stock_name(code)} 无法获取价格，跳过")
...
def init(ContextInfo):
	...
	tee = Tee(f"{g.log_path}\\{log_name}_all_weather.log")
	sys.stdout = tee
