...
def initialize(context):
    ...
    run_daily(stop_loss, time='09:45') # 止损函数
...
def judge_date(context):
    ...
    if g.reason_to_sell is 'entire_market_loss':
        log.info('今日大盘大跌，不进行交易')
        g.trade = False
        g.reason_to_sell = None
...
def check_remain_amount(context):
    if g.reason_to_sell is 'limitup': #判断提前售出原因，如果是涨停售出则次日再次交易，如果是止损售出则不交易
        ...
        if len(g.hold_list) < g.stock_num:
            ...
            g.refresh_hold = True
        g.reason_to_sell = ''
    elif g.reason_to_sell is 'stoploss' or g.reason_to_sell is 'entire_market_loss':
    ...
    if g.run_stoploss:
        ...
        if g.stoploss_strategy == 2 or g.stoploss_strategy == 3:
            ...
            if down_ratio >= g.stoploss_market:
                g.reason_to_sell = 'entire_market_loss'
...
def after_trading_end(context):
    current_date = context.current_dt.date()
    if current_date.weekday() != g.weekday and g.refresh_hold == False:  # 周一
        return
    g.refresh_hold = False
    #每日收盘后运行，记录当日持仓情况
    # 获取当前持仓
