...
def rebalance(context):
    ...
    if current_date.weekday() == 0 and current_month != g.last_month:
    ...
    if current_date.weekday() == 0:
        log.info(f"今日({current_date})为月初卖出日，执行卖出操作")
    ...
    # 判断是否为每月第二个交易日（买入日）
    elif current_date.weekday() == 1 and g.sell_done:
        log.info(f"今日({current_date})为月中买入日，执行买入操作")
        # 执行买入逻辑
        buy_stocks(context)
        # 重置卖出标记
        g.sell_done = False
...
def get_small_cap_stocks(stock_list, query_date, n=5):
    ...
    for idx, row in df_sorted.head(10).iterrows():
        ...
        log.info(f'    第{rank:>2}名: {stock_name}({row["code"]}), 流通市值: {cap_in_10k:.2f} 万元{marker}')
...
def sell_stocks(context):
    # 当前持仓的股票列表
    current_holdings = list(context.portfolio.positions.keys())
    stocks_to_sell = [s for s in current_holdings if s not in g.selected_stocks]
    ...
    if len(g.stocks_to_buy) > 0 or len(g.stocks_to_sell) > 0:
        ...
        log.info(f"需要卖出股票 {len(stocks_to_sell)}只")
