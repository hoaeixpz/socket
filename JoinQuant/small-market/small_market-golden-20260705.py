...
def info_position(context):
	...
	if len(positions) > 0:
		...
		for stock, pos in sorted_pos.items():
			...
			ratio = (price / avg_cost - 1) * 100
			diff_price = price - avg_cost
			cangwei = pos.value / context.portfolio.total_value * 100
			log.info('✅持仓: %s(%s), 占比 %.2f%%, 涨跌幅: %.2f%% (%.2f), 数量: %d, 市值: %.2f元' % 
					(stock_name, stock, cangwei, ratio, diff_price*pos.total_amount, pos.total_amount, pos.value))
